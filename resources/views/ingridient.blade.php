            ...
            <div class='w-full rounded-xl bg-white h-fit mx-auto'>
                <div class="p-3">
                    <div class="flex justify-between">
                        <h1 class="font-extrabold text-3xl">Ingridient</h1>
                        <a class="p-2 bg-blue-500 rounded-xl text-white hover:text-black text-center"
                            href="{{ route('addingridient') }}">Add
                            ingridient</a>
                    </div>
                </div>
                <div class="p-2">
                            ...
                            <tbody>
                                ...
                                @foreach ($menus as $item)
                                    <tr class="border-2">
                                        ...
                                        <td>{{ $item->name }}</td>
                                        <td>
                                            @forelse ($item->invents as $invent)
                                                • {{ $invent->name }}
                                                ({{ $invent->pivot->quantity_used }} {{ $invent->unit }})
                                                <br>
                                            @empty
                                                <span class="text-gray-500 italic">No ingredient</span>
                                            @endforelse
                                        </td>
                                        <td class="flex gap-2">
                                            <div class="w-full">
